...
int SMelodyFeatureExtraction(char* filename, float*& pFeaBuf,int &nFeaLen, SNote *&Query, int &nNoteLen,float ratio){
	/* 0. pitch extraction */
	...
	SProcessQuery(pFeaBuf,nFeaLen,ratio);
	if(nFeaLen<20){
		printf("Too short! please try again\n");
		if(NULL!=pFeaBuf){
			delete[] pFeaBuf;
...
int STester(char* szModel, char* szModelinfo, char* szWav, char* szOut){
	...
	int nNoteLen=0;
	SMelodyFeatureExtraction(szWav,pFeaBuf,nFeaLen,QueryNotes,nNoteLen,0.1);	
