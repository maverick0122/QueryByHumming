...
void Mean8Minutes( vector<float>  &x, int thd);	//减均值操作。加入thd参数，最多计算前thd帧的均值。
...
double MyMinTwo(double a, double b);//两个值比较大小
...
void MeanNew(vector< vector<double> > &x);//5秒内取平均
...
void MeanDimentionLSH(vector<float> &x);	//减均值
