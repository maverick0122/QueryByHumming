...
int WaveRead(char* filename, SWaveDataStru &mySWaveDataStru){
	...
	nRead=fread(pBuffer, sizeof(unsigned char), nLen, fp);
	fclose(fp);
	...
	for(int i=0;i<nLen;i++){
		//printf("%f  ",pBuffer[i]);
		mySWaveDataStru.fDataBuf[i]=(float)(pBuffer[i]-128)/128.0f;
		//printf("%f  ",mySWaveDataStru.fDataBuf[i]);
	}
...
void SProcessQuery(float* fPitchArray,int& Len,float ratio){
	int i;
	float MeanVal;
	...
	//for (i=0;i<Len-3;i++){
	//	if (i%3==0){
	//		fPitchArray[i/3]=GetMean(fPitchArray+i,3);	
	//		nCountFrm++;
	//	}
	//}
	...
	//for (i=0;i<Len-2;i++){
	//	if (i%2==0){
	//		fPitchArray[i/2]=GetMean(fPitchArray+i,2);	
	//		nCountFrm++;
	//	}
	//}
